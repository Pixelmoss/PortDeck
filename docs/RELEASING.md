...
npm run desktop:verify:bundle -- release/mac-universal/PortDeck.app
...
`desktop:verify:bundle` 可在没有 Apple 证书时先验证版本、CPU 架构、ASAR 和中英文资源完整性；它不代表签名或公证通过。

若需要在不覆盖现有 `release/mac-*` 测试应用的情况下隔离构建，可设置 `PORTDECK_BUILD_OUTPUT_DIR` 指向临时输出目录。
