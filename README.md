...
当前版本是可运行的 `0.3.0` Electron 桌面版本。扫描、状态模型、进程管理和 UI 主流程可以独立在浏览器中运行，也可以内嵌到 PortDeck macOS 应用中。

## 下载

Apple Silicon Mac 可从 [PortDeck 0.3.0 Release](https://github.com/Pixelmoss/PortDeck/releases/tag/v0.3.0) 下载 `.dmg` 安装包或 `.zip` 压缩包。
...
构建产物位于 `release/`。0.3 版本用于本机开发和内部测试；面向其他用户分发前仍需配置 Developer ID 签名、公证和正式应用图标。
